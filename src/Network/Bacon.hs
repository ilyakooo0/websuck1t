...
type API = QueryParam "type" LoremType :> QueryParam "paras" Int :> QueryParam "start-with-lorem" Int :> Get '[JSON] [Text]

clientAPI :: Proxy API
clientAPI = Proxy

loremBacon' :: Maybe LoremType -> Maybe Int -> Maybe Int -> ClientM [Text]
loremBacon' = client clientAPI

loremBacon :: Manager -> Maybe LoremType -> Maybe Int -> Maybe Int -> IO (Maybe [Text])
